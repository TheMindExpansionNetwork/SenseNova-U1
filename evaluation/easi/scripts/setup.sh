...
if [ "${SKIP_LIGHTLLM}" = "1" ]; then
  ...
  log "      python scripts/submissions/run_easi_eval.py --model SenseNova-U1-Mini-Beta-Local --benchmarks blink"
else
  log "  - launch server (weights auto-downloaded on first call):"
  log "      bash evaluation/easi/scripts/serve.sh                 # mini-beta → port 8000"
  log "      MODEL=mini-sft bash evaluation/easi/scripts/serve.sh  # mini-sft  → port 8001"
